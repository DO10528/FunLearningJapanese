...
function startSpeechRecognition() {
    ...
    // 1. 強制リセット処理: 録音中にマイクが再度押された場合は、stop() ではなく abort() で確実にシステムを強制終了させること
    ...
    // 2. 起動時の衝突回避: 起動前に必ず window.speechSynthesis.cancel() でTTSをリセット
    ...
    // 3. マイクの初期化: 毎回マイクを起動するたびに recognition = new SpeechRecognition(); を実行
    ...
    // 4. 待機時間のカット: recognition.onspeechend = () => { recognition.stop(); };
    ...
    // その後 setTimeout を使用して100ms待機してから recognition.start() を実行
    setTimeout(() => {
        ...
        try {
            ...
            if (e.name !== 'InvalidStateError') {
                resText.textContent = "エラー: 再読み込みしてください";
            }
        }
    }, 100);
}
