document.addEventListener('DOMContentLoaded', () => {
    console.log('Treasure Hunt loaded');

    const POINTS_PER_CORRECT = 1; 
    const ASSETS_BASE = 'assets/';

    const GAME_DATA = {
        stationery: { title: 'ぶんぼうぐ', color: '#ffd54f', words: [
        ...
        food: { title: 'たべもの', color: '#ffab91', words: [
        ...
        clothing: { title: 'おようふく', color: '#81d4fa', words: [
        ...
        toys: { title: 'おもちゃ', color: '#a5d6a7', words: [
        ...
        room: { title: 'おへや', color: '#ce93d8', words: [
        ...
        electronics: { title: 'かでん', color: '#b0bec5', words: [
        ...
        colors: { title: 'いろ', color: '#f48fb1', words: [
        ...
        body: { title: 'からだ', color: '#ffccbc', words: [
    ...
    function showScreen(screenId) {
        ...
        const allScreens = document.querySelectorAll('.sc-screen');
    ...
    function initTopicScreen() {
        ...
        Object.keys(GAME_DATA).forEach(key => {
            ...
            card.onclick = (e) => {
                e.preventDefault();
                e.stopPropagation();
                console.log('Topic selected:', key);
                selectTopic(key);
            };
            topicGrid.appendChild(card);
        });
        
        showScreen('sc-screen-topic');
    }

    function selectTopic(key) {
        console.log('selectTopic called with:', key);
        ...
        if (takePicBtn) {
            takePicBtn.onclick = (e) => {
                ...
                if (canvasEl && videoEl && currentWord && currentTopicId) {
                    ...
                    stopCamera();
                    showScreen('sc-screen-result');
                }
            };
        }

        if (cancelCameraBtn) {
            cancelCameraBtn.onclick = (e) => {
                e.preventDefault();
                e.stopPropagation();
                stopCamera();
                showScreen('sc-screen-game');
